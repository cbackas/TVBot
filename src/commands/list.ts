...
export const command: CommandV2 = {
  ...
  async executeCommand (app: App, interaction: ChatInputCommandInteraction) {
    ...
    const subCommand = interaction.options.getSubcommand() as typeof subCommands[keyof typeof subCommands]
    ...
    const showMessages = showsInChannel.map(show => {
      const destinations = show.destinations.map(destination => `<#${destination.channelId}>`).join(' ')
